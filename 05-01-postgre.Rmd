...
Postgres can use `pgpass` file to store password for connecting to database without inputting password, the format is `hostname:port:database:username:password`.
...
1. Linux: `~/.pgpass`
    ...
    ```sh
    chmod 600 ~/.pgpass # otherwise, this file will be ignored
    ```
2. Windows: `%APPDATA%\postgresql\pgpass.conf`
3. `PGPASSFILE` can be set to refer to the file
...
* `-c --command=COMMAND`: run single command (SQL or internal) and exit
* `-f --file=FILENAME`: execute commands from file, then exit
* `-l --list`: list available databases, then exit
* `-o, --output=FILENAME`
* `-t, --tuples-only`: output only, without account messages
* `-A, --no-align`: unaligned table output mode, without `|` separator
* `-F, --field-separator=STRING`: default `|`
* `-z, field-separator-zero`: set field separator to ASCII 0
* `-R, --record-separator=STRING`: default newline
* `-0, record-separator-zero`: set record separator to ASCII 0
