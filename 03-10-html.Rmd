...
1. Widget
    ...
    2. `<form>`
        ...
        3. `novalidate`: turn off the browser's automatic validation
    3. `<input>`: a single line text field
        1. `type`
            1. primitive
                ...
                6. `file`
                    ...
                    2. `accept="image/*"`
                7. `submit`
                5. `image`: submit using image
                6.  `button`: have no automatic effect like `submit`, but can be customized using js code
                7.  `reset`: not recommended
            2. HTML5
                1. `number`
                    1. `min="10"`
                    2. `max="30"`
