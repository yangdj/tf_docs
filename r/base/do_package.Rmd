...
* a `man/` directory to hold documents
...
* `Depends:` - `Imports` just loads the package, `Depends` attaches it. There are no other differences. use `usethis::use_package("dplyr", "Depends")` to add the needing `Depends` package
...
```r
    ...
    devtools::install_github('cats','github_username')
```
