...
5. Kafka
    ...
    3. Configurations
        ...
        2. `config/connect-distributed.properties`
            ...
            5. `key.converter.schemas.enable=true`: default
            6. `value.converter.schemas.enable=true`: default
            ...
            10. config storage
                ...
                3. Commands
                    1. `kafka-topics.sh --create --topic connect-configs --bootstrap-server hpmaster:9092 --partitions 1 --replication-factor 2 --config cleanup.policy=compact`
            11. offset storage
                1. Notes
                ...
                3. Commands
                    1. `kafka-topics.sh --create --topic connect-offsets --bootstrap-server hpmaster:9092 --partitions 10 --replication-factor 2 --config cleanup.policy=compact`
            12. status storage
                1. Notes
                ...
                3. Commands
                    1. `kafka-topics.sh --create --topic connect-status --bootstrap-server hpmaster:9092 --partitions 10 --replication-factor 2 --config cleanup.policy=compact`
    4. Services
        1. start for every host: `kafka-server-start.sh [-daemon] <config/server.properties>`
        2. stop for every host: `kafka-server-stop.sh <config/server.properties>`
        3. topic:
            1. list: `kafka-topics.sh --list --bootstrap-server localhost:9092`
            2. create: `kafka-topics.sh --create --topic quickstart-events --bootstrap-server localhost:9092`
                1. `--topic <topic-name>`
                2. `--replication-factor <num>`
                3. `--partitions <num>`
            3. describe: `kafka-topics.sh --describe --topic quickstart-events --bootstrap-server localhost:9092`
            4. delete: `kafka-topics.sh --delete --topic quickstart-events --bootstrap-server localhost:9092`
        4. connectors
...
6. Hive
    ...
    2. Envs
        ...
        ```sh
        export HIVE_HOME=/data/hadoop/hadoop/bin/hive/apche-hive-3.1.2
        export PATH=${HIVE_HOME}/bin:${PATH}
        ```
    3. Configurations
        1. `HIVE_CONF_DIR/hive-site.xml`
            1. `hive.metastore.warehouse.dir: /user/hive/warehouse`: location of default database for the warehouse
            ...
            5. `javax.jdo.option.ConnectionDriverName`: put driver jar in the folder `HIVE_HOME/lib`
                1. mysql: `com.mysql.cj.jdbc.Driver`
            6. `javax.jdo.option.ConnectionUserName: db_username`
            7. `javax.jdo.option.ConnectionPassword: db_password`
    ...
    4. Services
        1. initilize database: `schematool -initSchema -dbType mysql`
            1. `Exception in thread "main" java.lang.NoSuchMethodError: com.google.common.base.Preconditions.checkArgument(ZLjava/lang/String;Ljava/lang/Object`: replace `apache-hive-3.1.2-bin/lib/guava-19.0.jar` with `hadoop-3.2.2/share/hadoop/common/lib/guava-27.0-jre.jar` 
        2. use interactively: `$HIVE_HOME/bin/hive`
        3. start server, default port `10000`: `$HIVE_HOME/bin/hiveserver2`
        4. connect server: `$HIVE_HOME/bin/beeline -u jdbc:hive2://$HS2_HOST:$HS2_PORT`
7. Scala
    1. Install
