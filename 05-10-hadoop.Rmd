...
6. Hive
    ...
    3. Configurations
        1. `HIVE_CONF_DIR/hive-site.xml`
            ...
            8. `hive.metastore.event.db.notification.api.auth: false`
            9. `hive.server2.enable.doAs: false`
            10. `[metastore.thrift.port: <default: 9083>]`
            11. `[hive.server2.thrift.port: 10000]`
            12. `[hive.server2.thrift.bind.host: hpmaster]`
    ...
    4. Services
        ...
        3. connect server
            1. beeline: `$HIVE_HOME/bin/beeline -u jdbc:hive2://hpmaster:10000 -n username -p passport`
                1. `Concurrency mode is disabled, not creating a lock manager`: in `hive-site.xml`
...
7. Tez
    1. Install
        ...
        ```sh
        TEZ_VERSION=0.10.0
        wget https://mirrors.tuna.tsinghua.edu.cn/apache/tez/0.10.0/apache-tez-${TEZ_VERSION}-bin.tar.gz
    2. Envs

        ```sh
        export TEZ_HOME=HIVE_HOME=/data/hadoop/hadoop/bin/hive/apche-tez-0.10.0-bin
        export TEZ_CONF_DIR=/server/proj/hp/config/tez
        ```
    3. Configurations
        1. put `${TEZ_HOME}/share/tez.tar.gz` to hdfs
            1. `hadoop fs -mkdir /user/tez`
            2. `hadoop fs -put tez.tar.gz /user/tez`
8. Scala
    1. Install
