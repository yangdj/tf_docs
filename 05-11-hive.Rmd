...
5. SQL
    ...
    2. Functions
        1. Query
            1. `SHOW FUNCTIONS`
            2. `DESC FUNCTIONS func_name`
            3. `DESC FUNCTIONS EXTENDED func_name`
        2. UDFs: user defined functions
            1. General
                1. `version()`
                2. `cast(from_type as another_type)`
                3. `greatest(v1, v2, ...)`
                4. `least(v1, v2, ...)`
            2. str
                1. `split(str, regex)`
                2. `concat_ws(sep, [str | array(string)]+))`
            3. Timestamp
                1. `current_timestamp()`
                2. `unix_timestamp([timestamp, 'yyyy-MM-dd hh:mm:ss])`
                3. `from_unixtime(unixtime[, 'yyyy-MM-dd hh:mm:ss'])`
            4. Date
                1. `current_date()`
                2. `to_date(timestamp)`
                3. `date_add(start_date, days)`
                4. `date_sub(start_date, days)`
                5. `datediff(end_date, start_date)`
                6. `last_day()`: last day of month
