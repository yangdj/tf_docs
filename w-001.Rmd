...
4.  `Alt+<-, Alt+->`: go to other file
5. `Ctrl+Click`: go to link or definition
6. `Ctrl+G;`: go to line
7. `Ctrl+Shift+\`: jump between brackets
8. `Ctrl+Shift+O`: jump to symbols
9. `Ctrl+[, Ctrl+]`: line indent
10. `Shift+Alt`: column selection
11. `Ctrl+D`: select current or next word under the cursor
12. `Ctrl+Shift+L`: select all the same word uner the cursor
13. `Ctrl+Space`: trigger autocompletion
14. `Alt+Click`: add new cursor
15. `Ctrl+Backspace`: delete left word
16. `Ctrl+Delete`: delete right word
17. `Ctrl+Shift+K`: delete current line
18. `Ctrl+Enter`: insert line below
19. `Ctrl+Shift+Enter`: insert line above
20. `Alt+Uparrow, Alt+Downarrow`: move line up and down
21. `Alt+Shift+Uparrow, Alt+Shift+Downarrow`: copy line up and down
...
4. `[]`: match one of characters group
   1. `example.[0-9]` matches `example.0, example.1, ...`
