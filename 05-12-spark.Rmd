...
1. References
    ...
    2. [Structured Streaming + Kafka Integration Guide](https://spark.apache.org/docs/2.1.1/structured-streaming-kafka-integration.html)
    3. [spark-examples](https://github.com/apache/spark/blob/master/examples/src/main/scala/org/apache/spark/examples/streaming/DirectKafkaWordCount.scala)
2. Concepts
    1. driver
...
4. Data Structure
    ...
    4. `org.apache.spark.sql.types.StructType`
        ...
        2. `val schema = new StructType().add("name", StringType, true)...`
            ...
            3. `jdbc`
                1. All Options
                    1. `.option("url", "jdbc:postgresql://host:port/dbname?user=u&password=p")`
                        1. PostgreSQL: `jdbc:postgresql://host:port/dbname?user=u&password=p`
                        2. MySQL: `jdbc:mysql://host:port/dbname?user=u&password=p`
                    ...
                    5. `.option("driver","org.postgresql.Driver")`: when `spark-shell` or `spark-submit` speicify `--driver-class-path`, no need to config this option
                    ...
                    7. `.jdbc("url", "schema.table", connectionProperties)`
                        1. `val connectionProperties = new java.util.Properties()`
                        2. `connectionProperties.put("user", "username")`
                        3. `connectionProperties.put("password", "password")`
                        4. `connectionProperties.put("driver", "com.mysql.cj.jdbc.Driver")`
    ...
    2. Kafka
        ...
        5. Write
            ...
            3. Batch: use `write` instead of `writeStream` 
                1. `val ds = df.selectExpr("CAST(key AS STRING)", "CAST(value AS STRING)").write.format("kafka").option("topic", "topic1").start()`: specify a topic in an option
                2. `val ds = df.selectExpr("topic", "CAST(key AS STRING)", "CAST(value AS STRING)").write.format("kafka").start()`: specify a topic in the data
    3. Files
