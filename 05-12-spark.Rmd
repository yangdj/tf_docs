    ...
    5. `org.apache.spark.rdd.RDD`
        ...
        3. Initialization
            1. From Driver
                ...
                1. `val peopleRDD = spark.sparkContext.makeRDD(data)`
                2. `val peopleRDD = spark.sparkContext.parallelize(data)`
        ...
        5. Actions: return a value to the driver program after running a computation on the dataset
            ...
            1. `.take([n])`: fetch first `n` number of elements, safer then `.collect()` when dataset is large
            2. `.count()`
            1. `.first()`
            2. `.reduce()`
            3. `.foreach()`
            7. `.countByKey()`
        5.  Cache: persist an RDD datasets into a cluster-wide in-memory cache
            1. `.persist()`
        ...
        6.  Shared Variable
            ...
            2. Accumulator
                1. Properties
                    ...
                    1. natively supports accumulators of numeric types
                    1. tasks running on a cluster can add to it using `add()` method, but they cannot read its value.
                    2. only the driver program can read the accumulator's value, using `value` method
                    3. performed inside **actions only**, Spark guarantees that each task’s update to the accumulator will only be applied once, restarted tasks will not update the value. In transformations, each task's update may be applied more than once if re-executed
                2. type
    ...
    6. `org.apache.spark.sql.Dateset`
        ...
        2. Initialization
            1. From Driver
                ...
                1. `val peopleDS = spark.createDateset(List(Person("Andy", 32), Person("Bob", 12)))`
            1. From Spark
                1. `spark.range(100)`
            2. From RDD
        ...
        3. Metadata
            ...
            1. `.schema`
            3. `.printSchema`
            1. `.explain([true, false])`: `[default: false]`
        4. Type-Safe
            1. Transforms
    ...
    7. `org.apache.spark.sql.DataFrame`
        ...
        2. Initialization
            ...
            2. From Files
                ...
                1. `val userDF = spark.read.load("users.parquet")`
                2. `val userDF = spark.sql("select * from parquet.users.parquet")`
            3. From RDD
...
5. Source
    1. Methods
        1. Load
            ...
            5. `val peopleDF = spark.read.format("csv").option("sep", ";").option("header", "true").option("inferSchema", "true").load("people.csv")`
            6. `val peopleDF = spark.read.format("json").load("people.json")`
            7. `val peopleDF = spark.read.format("parquet").load("dir1/", "dir2/")`
            8. `val peopleDF = spark.read.load("people.parquet")`
            9.  ``val sqlDF =spark.sql("select * from parquet.`people.parquet`")``
        2. Save
            1. `peopleDF.write.format("csv").option("header", "true").mode(modeValue).save("users.csv")`
            2. `peopleDF.write.parquet("people.parquet")`
            3. `peopleDF.write.parquet("data/people/key=1")`
            4. `peopleDF.write.save("people.parquet")`
            5. `peopleDF.write.partitionBy("color").bucketBy(10, "name").sortBy("age").saveAsTable("t_par_but")`: save to table
            6. `peopleDF.write.format("orc").option("orc.bloom.filter.columns", "favorite_color").option("orc.dictionary.key.threshold","1.0").option("orc.column.encoding.direct", "name").save("users_with_options.orc")`
    2. Configurations
