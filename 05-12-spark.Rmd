...
1. References
    ...
    4. [Spark to PostgreSQL](https://3rdman.de/2019/08/spark-to-postgresql-real-time-data-processing-pipeline-part-5/)
2. Concepts
    1. driver
            ...
            3. `jdbc`
                1. All Options
                    1. `.option("url", "jdbc:postgresql://host:port/dbname?user=u&password=p")`
                        ...
                        6. SQLServer 2000: `jdbc:sqlserver://host:port;database=dbname;user=u;password=p`
                    ...
                    5. `.option("driver","org.postgresql.Driver")`: when `spark-shell` or `spark-submit` speicify `--driver-class-path`, no need to config this option
                        ...
                        3. Hive: `org.apache.hive.jdbc.HiveDriver`
...
6. Structured Streaming
        ...
        2. Load
            1. Options
                1. `.option("subscribe", "topic")`
                    1. `topic1`: one topic
                    1. `topic1,topic2`: multiple topics
                    2. `topic.*`: a pattern, java regex string
                2. `[.option("startingOffsets", "latest")]`
                    ...
                    2. `"""{"topic1": {"0":23, "1": -1"}, "topic2":{"0":-2}}"""`: json string specifying a starting offset for a topic partition
    ...
    4. Hbase
        1. Dependencies  
            1. `org.apache.hbase" % "hbase-client" % "2.3.5"`
            2. `org.apache.hbase" % "hbase-common" % "2.3.5`
