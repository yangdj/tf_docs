# Idea

1. Settings
    1. Code
        1. File Encoding: `Editor/File Encoding: UTF-8`
        2. Line Separator: `Editor/Code Style/Line separator: \n`
    2. Shortcuts
        1. `Ctrl + click`: jump to definition
        2. `Ctrl + Shift + N`: open file
        3. `Ctrl + Alt + L`: format code
        4. `Ctrl + /`: line comment
        5. `Ctrl + Shift + /`: block comment
    3. Imports: `Code Style.java.Imports`
        1. `Class count to use import with '*': 500`
        2. `Names count to use static import with '*': 500`
    4. Plugins
        1. IdeaVim
            1. `~/.ideavimrc`
    5. Project
        1. Scala
            1. download scala plugin
            2. may need `Add Framework Support for scala` when not see `Scala Class` creating scala class file, just right click project oolder
            3. build sbt
                1. JRE: sbt install home
                2. VM parameters: `-Duser.language=en`
