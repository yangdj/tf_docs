...
1. Configurations
    1. Settings
        ...
        2. Options
            ...
            6. `git config --global core.excludesfile <~/.gitignore>`: if not specified, then default `$XDG_CONFIG_HOME/git/ignore`, if `$XDG_CONFG_HOME` not set or empty, use `$HOME/.config/git/ignore`
    2. Repo's Structure
        1. Project/
            1. `.git/`: auto created
            2. `.gitignore`
    ...
    3. Ignorance: specify untracked files to ignore
        1. Rules
            ...
            5. trailing spaces are ignored unless they are quoted with backslash `\ `
            6. `!` negates the pattern, **but if parent directory has been excluded, it will not work**, literal `!` can be excaped, `\!`
            7. `/` serves as a directory separator
                1. at the end of the pattern, the pattern will only match directories, otherwise match both files and directories
                2. at the beginning or middle (or both) of the pattern, the pattern is relative to the directory level of the particular `.gitignore` file itself
                3. Otherwise the pattern may match at any level below the `.gitignore` level
            8. `*` matches anything except `/`
            9. `?` matches anything except `/`
            10. `[a-zA-Z]` matches a range
            11. the last matching pattern wins
        2. Files
...
2. Commands
    ...
    10. `git diff`
        ...
        3. `git diff {--cached, --staged}`: diff between staged and HEAD (last commit)
        4. `git diff HEAD`: diff between working tree and HEAD (last commit)
    ...
    13. `git pull`: fetch remote updates and merge with them
    ...
    14. `git push`: push local updates to remote
        ...
        7. `git push -u <remote> <local-branch>`: set a default tracking remote branch if current local branch has no or multiple tracking branches, equivalent to `git push origin master; git branch --set-upstream master reomte/master`
