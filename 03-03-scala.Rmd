...
7. Exception
    ...
    4. Exception Handlers
        1. try-catch-finally
            ...
            3. `finally`
        2. try-with-resources: since Jave7, suit to situations using `Closeable` resources
    5. `throw new IllegalArgumentException`: `throw` has type `Nothing`
    6. `try{} catch{} finally {}`: returns a value, if exception thrown not caught, the expression has no result at all
    7. the value computed in the `finally` clause will be dropped, it usually does some clean work
    8. if `finally` has explicit return, or throws an exception, the return value or exception will the final expression result
