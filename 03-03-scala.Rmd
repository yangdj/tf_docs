...
3. Commands
    ...
    6. `help <command>`
    7. `clean`: clear output under `target/`
    8. `update`: download dependencies manually, used by `compile`, so always no need to execute it
    9. `compile`: files in `src/main/{scala, java}`
    10. `test`: compile and run test file
    11. `console`: enter scala shell with compiled environment
    12. `run [arguments]`: run main class
    13. `package`: generate a jar file from `scr/main/resources` and `scr/main/{scala,java}`,  need `name` and `version` in the file `build.sbt`
    14. `~<command>`: run the command automatically when files update, `~run`. `Enter` key will cancel it
    15. `reload`: reload build files, `build.sbt`, `project/{*.scala, *.sbt}`
4. Problems
    ...
    3. Complex
        ...
        2. String
            ...
            2. Methods
                ...
                15. `.trim()`
                16. `.drop(2)`
                17. `s"hello, $name, age is: ${6 * 7}"`: string interpolates
                18. `raw"no\\\\escape!" => no\\\\escape!`: raw string will not escape
                19. `f"${math.pi}%.5f"`: printf-style formatting, when no `"%"`, then default is "%s", the toString value
        3. List
            1. Literal
...
7. Exception
    ...
    ```
8. Package
    1. scala implicitly imports packages `java.lang`, `scala` and the members in the singleton object `Predef`
    2. `println` invokes `Predef.println`, which invokes `Console.println`
    3. `assert` invokes `Predef.assert`
    4. `import` allows using a simple package element name
