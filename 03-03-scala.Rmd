...
6. Commands
    ...
    16. `~<command>`: run the command automatically when files update, `~run`. `Enter` key will cancel it
...
6. Class
    ...
    3. Definition
        ...
        5. `scala.APP` Trait
            ...
            ```
7. Exception
    1. Catch or Specify Requirement: code that might throw exceptions must be enclosed by either of the following
    ...
    3. Types
        ...
        2. Checked Exception
            1. Exception
                1. Should anticipate and recover from these exceptions
                2. All exceptions are checked exceptions, except for `Error` and `RuntimeException` and their subclass
        ...
        3. Unchecked Exceptions
            ...
            2. Runtime Exception
                1. Internal to the application that cannot anticipate or recover from ndicating bugs such as logic error or improper use of API
    ...
    4. `throw new IllegalArgumentException`: `throw` has type `Nothing`
    5. `try{} catch{} finally {}`: returns a value, if exception thrown not caught, the expression has no result at all
    6. the value computed in the `finally` clause will be dropped, it usually does some clean work
    7. if `finally` has explicit return, or throws an exception, the return value or exception will the final expression result
