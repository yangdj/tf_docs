    ...
    1. `project_name/`
        ...
        3. `src/`
            1. `main/`
                ...
                3. `resources/`
                    1. Read File
                        1. `ClassName.class.getResourceAsStream("/a.txt")`: static method start with `/`
                        2. `getClass().getResourceAsStream("/a.txt")`: instance method
            2. `test/`
                1. `scala/`
                2. `java/`
                3. `resources/`
...
1. References
    ...
    2. [The Java Language Specification](https://docs.oracle.com/javase/specs/jls/se8/html/index.html)
    3. [Java Tutorial](https://docs.oracle.com/javase/tutorial/)
    4. [Type Wildcard in Java Generics](http://ted-gao.blogspot.com/search/label/Java%20Generics)
    5. [Java Concurrency](http://tutorials.jenkov.com/java-concurrency/volatile.html)
    6. [Singleton](https://en.wikipedia.org/wiki/Singleton_pattern#Lazy_initialization)
2. Command
    1. Common
        1. `jar`
    ...
    1. Java
        1. Variables
            1. Naming
                ...
                2. Styles
                    1. use camelCase
                    2. class name captitalizes the initial letter
                    3. constant value capitalizes every letter and separates subsequent words with underscore
                    4. method names should be a verb in lowercase or a multi-word name beginning with a verb in lowercase followed by adjectives, nouns, etc.
            ...
            3. Array: hold a fix number of values of a single type
                ...
                3. Operations
                    ...
                    2. Constructor: `anArray = new int[10]`
                    3. initialization: `anArray[0] = 100`
                    4. declaration and initialization: `int[] anArray = {100, 200, ..., 1000}`, length is determined by element number     in the brace separated by commas
                    5. access: `anArray[0]`
        ...
        1.  Modifier
            1. Access
                1. Class
                    ...
                    3. `private`: only applied to nested class
                2. Member
                    1. `public`: visible to all
                    2. no modifier: package-private
                    3. `private`: visible within its own class
                    4. `protected`: visible within its own package (package-private) and subclass of its class in another package
            2. Others
                1. `static`
                    1. Member: belong to class, not instances
                2. `final`: value can not be changed
                    1. variable: value can not be change
                    2. method: can not be overridden by subclasses. methods called from constructors should generally be declared final. If a constructor calls a non-final method, a subclass may redefine that method with surprising or undesirable results
                    3. class: cannot be extended. useful, for example, when creating an immutable class such as the class `String`
                3. `transient`: specify variable not to be serialized
                4. `default`: for interface's default method
        2. Class
            1. Member
                1. Variable
                    1. Instance Variable: fields, non-static fields, unique to each instance of class, can be initialized in constructors
                        1. uninitialized default value
                            1. primitive: see primitive data types
                            2. objects: `null`
                    2. Class Variable: static fields, declared with `static` modifier, only one copy of this variable, can be initialized in static initialization blocks or through a private static mathod
                        ...
                        ```
                2. Method
                    1. Parameters: variables in method declaration
                    2. Arguments: actual values passed to the invoked method. When pass an arbitary number of values to a method, use varargs following the type of last parameter by an ellipsis (three dots, `...`), then a space and the paremeter name and the parameter is treated as an array
                        ...
                        ```
                    3. Return
                        1. Type: value returned by the method, or `void` if the method does not return a value
                        2. When
                            1. complete all the statements
                            2. reach a `return` statement
                            3. throw an exception
                    4. Local Variables: variables in a method body
                    ...
                    5. Signature
                        1. a method's name and the parameter types comprise method signature
                        2. different signatures distinguish different methods
                        3. the return type is not considered
                    6. Overloading
                        1. a method has same name as others, but have different parameters
                        2. different parameters means the number or types are different
                3. Constructor: create objects from the class blueprint, like method declarations
                    1. use class name and have no return type
                    2. use `new` to create a new object
                    3. default constructor
                        1. The compiler automatically provide a no-argument constructor for any class without constructors
                        2. This default constructor will call the no-argument constructor of its superclass
                        3. The compiler will complain if the superclass doesnot have a no-argument constructor
                        4. If a class has no explicit superclass, then it has an implicit superclass `Object`, which has a no-argument constructor
                    4. `this()` method can invoke other constructor, but should be the first line of in the constructor
        3. Nested Class
            1. defined within class
                1. static nested class
                    1. declared with `static`
                    2. a member of its enclosing class, can not access to other members of the enclosing class because it is an instance of the enclosing class, but only access static members of the enclosing class
                    3. can be declared `private`, `public`, `protected` and package private with no modifier

                    ```java
                    StaticNestedClass staticNestedObject = new StaticNestedClass()
                    ```
                2. inner class
                    1. non-static class, associate with an instance of its enclosing class
                    2. cannot define any static members itself
                    3. can access to other members of the enclosing class, even if they are private

                    ```java
                    OuterClass outerObject = new OuterClass();
                    OuterClass.InnerClass innerObject = outerObject.new InnerClass();
                    ```
            2. defined within method: access modifiers apply
                1. local
                    1. local classes are non-static because they have access to instance members of the enclosing block
                    2. local classes cannot define or declare any static members similar to inner class
                    3. local classes can have static members privided that they are constant variables (a variable of primitive type or string that is defined final and initialized with a compile-time constant expression)
                    4. local classes in static methods can only refer to static members of the enclosing class
                    5. local classes can access to the members of its enclosing class
                    6. local classes can access the method's parameters since Java SE8
                    7. local classes can access local variables and parameters of the enclosing block since that are final or effectively final Java SE 8. A variable or parameter whose value is never changed after it is initialized is effectively finala
                    8. an interface cannot be declared inside a block; interfaces are inherently static
                2. anonymous
                ...
                3. lambda expression
                    1. express a single-method classes more compactly
                    2. work like an anonymous methods, methods without a name
                    3. method references, e.g., `Person::compareName`

                    ```java
                    public static void printpersionWithPredicate(List<Persion> roster, Predicate<Persion> tester) {
                        for (Person p: roster) {
                            if (tester.test(p)) {
                                p.printPerson();
                            }
                    ...
                    printpersionWithPredicate(roster, p -> p.getAge >= 10)
                    ```
        4. Inheritance and SubClass
            1. Properties
                1. use `extends` to extend its superclass
                2. except class `Object` which has no superclass, every class has one and only one direct superclass (single inheritance). In the absence of any other explicitly superclass, every class is implicitly a subclass of `Object`
            2. Members
                1. inherit all of the `public` and `protected` members of its parent, no matter what package the subclass is in
                2. inherit *package-private* members of the parent in the same package
                3. for `private` parent members
                    1. if the superclass has public or protected methods for accessing its private fields, these can also be used by the subclass
                ...
                4. declare fields with the same name as the ones in the superclass, type can be different, thus **hiding them** not recommended for it makes code difficult to read
                5. declare new fields not in the superclass
                6. write new *insance* methods have the same signature as the ones in the superclass, thus **overriding them**. It can return a subtype of the type returned by the overridden method. This subtype is called a *covariant return type*
                7. static methods in interfaces are never inherited
                8. write new *static* methods have the same signature as the ones in the superclass, thus **hiding them**
                9. write new methods not in the superclass. write overloaded methods inherited from the superclass. such overloaded methods neither hide nor override the superclass instance methods, they are new methods, unique to the subclass
                10. a compile-time error occurs when to change an instance method in the superclass to a static method in the subclass, and vice versa
                11. write subclass constructors that invoke superclass's constructor using keyword `super`. if a constructor does not explicitly invoke a superclass constructor, the Java compiler automatically inserts a call to the no-argument constructor of the superclass. if the super class does not have a no-argument constructor,  a compile-time error occurs. `Object` does have such a constructor, so if `Object` is the only superclass, there is no problem
                12. the access specifier for an overriding method can allow more, but not less, access than the overridden method. e.g., a protected instance method in the superclass can be made public, but not private, in the subclass
            3. Operations
                1. `instanceof`: operator to test the type of an object, `obj instanceof Bicycle`
                2. `super.methodA()`: invoke superclass's method
                3. `ClassA.super.methodA()`: invoke specified superclass's method
        5. Abstract Methods and Classes
            1. Abstract Classes
                1. declared `abstract`
                2. can declare fields that are not static and final, and use all kinds of access modifiers
                3. can declare `statci` fields and methods
                4. may or may not include abstract methods
                5. if a class includes abstract methods, then the class itself must be declared abstract
                6. can not be instantiated, but can be subclassed
                ...
                8. no need to implement all of the interface's methods when declared to be `abstract` class
            2. Abstract Methods: declared `abstract` without an implementation
        6. Interface
            1. Properties
                1. cannot be instantiated
                2. can only be implemented by classes and extended by other interfaces
                3. only default and static methods can have implementations
                4. all methods in an interface are implicitly `public`
            2. Members
                1. constants:  implicitly `public`, `static`, and `final`
                2. method signatures: abstract methods, have no bodies and are terminated with a semicolon
                3. default methods
                    1. use `default` modifier
                    2. add new functionality and ensure binary compatibility with code written for older versions
                    3. when extending interfaces that contain default mothods, there are three choices:
                        1. not mention the default method at all, letting the extended interface inherit the default method
                        2. redeclare the default, which makes it abstract again
                        3. redefine the default method overriding it
                4. static methods
                    1. use `stattic` modifier
                    2. easier to organize helper methods in libraries, keep static methods specific to an interface in the same interface rather than in a separate class
                5. nested types
        7. Enum
        8. Object
            1. `null`
            ...
            2. `this`: current object
            3. Members
                1. `.class`: refer to object's class
5. Data Structure
    1. Architecture
        1. Relations
            1. `scala.Any`
...
12. Libraries
    1. `java.lang`
        1. `Object`
            1. `.clone()`: create and copy this object
            2. `.hashCode()`
                1. hash value for the object, which is an integer value generated by a hashing algorithm
                2. by definition, if two objects are equal, their hash code must also be equal
                3. if override the `equals()` method, then it changes the way two objects are equated and `Object`'s implementation of `hashCode()` is no longer valid, so need override the `hashCode()` method as well
            3. `.equals()`
                1. indicate whether some other object is equal to this one
                2. The `equals()` method provided by the `Object` uses the identity operator `==` to determine whether two objects are equal (the object references are equal). For primitive data types, this gives the correct result, but for objects, however, it does not.
                3. Class should always override the `equals()` method if the identity operator is not appropriate.
                4. if override `equals()`, must override `hashCode()` as well.
            4. `.finalize()`: called by garbage collector when determines there are no more references to the object
            5. `.getClass()`: get runtime class of the object, can not be overidden
            6. `.toString()`
                1. string representation of the object.
                2. should always consider overriding the `toString()` method
            7. `.notify()`
            8. `.notifyAll()`
            9.  `.wait()`
            10. `.wait(timeout)`
            11. `.wait(timeout, nanos)`
        2. `Class`
            1. `.getName()`
            2. `.getSimpleName()`
            3. `.getSuperclass()`
        ...
        3. System
            1. `System`
                1. `.exit(n)`: `System.exit(1)`
                    1. cause JVM to shutdown
                    2. the exit status is available to the process that launched the application
                    3. by convention, 0 indicates normal termination, while any other value is an error code
                2. `.arraycopy()`: `System.arraycopy(copyFrom, 2, copyTo, 0, 7)`
                3. `.currentTimeMillis()`
                4. `.nanoTime()`: time recorded by JVM, not for recording actual time, better for measuring time lapsed in nanoseconds
                5. `.getenv()`
                6. `.setProperties()`
                7. `.getProperties()`
                8. `.getProperty(name, [default])`
                    1. `"java.home"`
                    2. `"java.version"`
                    3. `"file.encoding"`
                    4. `"file.separator"`
                        1. `\` Windows
                        2. `/`: Linux
                    5. `"path.separator"`: character used in java.class.path `;` for windows
                    6. `"line.separator"`
                    7. `"user.name"`
                    8. `"user.home"`: user home directory
                    9. `"user.dir"`: user working directory
                    10. `"user.language"`: `zh`
                    11. `"user.timezone"`
                        1. `Asia/Shanghai`
            2. `System.in` 
        ...
        5. `String`
            ...
            3. Members
                ...
                11. `.replace()`
                12. `.replaceAll()`
                13. `.substring(start, end)`:  between `.charAt(start)` and `.charAt(end)`
                14. `.sbustring(start)`: equal `.substring(start, .length() -1)`
                15. `.toCharArray()`
        6. `Math`
            1. `Math.pow(a, b)`: `a^b`
    2. `java.math`
    ...
    3. `java.time`
        ...
        2. `LocalDate`
            1. `LocalDate.now()`
            2. `LocalDate.from(datetime)`
            3. `LocalDate.of(day, month, year)`
            4. `.plusYears(1)`
            5. `.getDayOfWeek()`
        3. `LocalTime`
            1. `LocalTime.now()`
            2. `LocalTime.of(hour, minute, second)`
            3. `LocalDate.from(datetime)`
        4. `LocalDateTime`
            1. `LocalDateTime.now()`
            2. `LocalDateTime.of(date, time)`
        5. `ZonedDatetime`
            1. `ZonedDateTime.of(datetime, zoneid)`
        6. `ZoneId`
            1. Properties
                1. identified by *region/city*, e.g., `Asia/Shanghai`
            2. Methods
                1. `ZoneId.of(zoneString)`
                2. `ZoneId.systemDefault()`
                3. `ZoneId.getAvailableZoneIds()`
        7. `ZoneOffset`: offset from Greenwich/UTC time
        8. `DayOfWeek`: constants from 1 (Monday) through 7 (Sunday)
            1. Enums
                1. `DayOfWeek.MONDAY`
                2. `DayOfWeek.TUESDAY`
                ...
                7. `DayOfWeek.SUNDAY`
            2. Methods
                1. `getDisplayName(textstyle, locale)`
        9. `Month`
            1. Enums: from 1 (January) to 12 (December)
                1. `JANUARY`
                2. `...`
        10. `format.TextStyle`
            1. Enums
                ...
                3. `SHORT`
    4. `java.util`
        1. `Properties`: both key and value are `String` values
            1. `.load(inStream)`
            2. `.store(outStream, comments)`
            3. `.size()`
            4. `.getProperty(key)`
            5. `.getPorperty(key, default)`
            6. `.propertyNames()`
            ...
            11. `.containsKey(key)`
            12. `.list(outStream)`
            13. `.setProperty(key, value)`: invoke `put()` from `Hashtable`
            14. `.remove(key)`
            15. `.clear()`
        2. `Calendar`
            1. Members
        ...
        5. `Arrays`
            1. `Arrays.asList()`
            2. `Arrays.equals()`: `Arrays.equals(chars, "123".toCharArray())`
            3. `Arrays.fill()`: `Arrays.fill(name, ' ')`
            4. `Arrays.copyOfRange()`: `String[] copyTo = java.util.Arrays.copyOfRange(copyFrom, 2, 9);`, include start, exclude end
            5. `Arrays.binarySearch()`
            6. `Arrays.sort()`
            7. `Arrays.parallelSort`: since Java SE8, concurrently execute
        6. `List`
        7. `ArrayList`
            1. Constructor
    ...
    5. `java.util.function`
        1. `Predicate<T>`
            1. `boolean test(T t)`
        2. `Predicate<T, R>`: return an r object from apply method to t
            1. `R apply(T t)`
    6. `java.io`
        1. Byte Streams
            1. `InputStream`
    ...
    7. `java.nio`: since Jave SE7
        ...
        2. `java.nio.file`
            1. `Path`
                1. Properties
                    1. store names as a sequence, from the highest element in the directory stucture with index 0 (the element closet to the root, `home` in `/home/tom`), and the lowest element  `tom` with index `n-1`
                2. Members
...
3. `Map`
    1. `AbstractMap`
        ...
        2. `TreeMap`
        3. `WeakHashMap`
    2. `HashTable extends Dictioanry`
    3. `SortedMap`
        1. `NavigableMap`
            1. `TreeMap`
