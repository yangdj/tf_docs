...
5. Commands
    ...
    4. SSH
        1. Operations
            1. `ssh-keygen -t rsa`: create keys, stored default `~/.ssh` directotry
                1. `id_rsa.pub`: public key file, put this key to remote `authorized_keys` file, then login without password
                2. `id_rsa`: private key file, must keep safely
                3. `authorized_keys`: file storing login's public key, must have **mode 600**, otherwise will not work
        ...
        2. Configurations: see `man ssh_config`
            1. Files
