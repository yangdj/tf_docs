...
2. Concepts
    ...
    5. Modules: the units of code ansible executes, since ansible 2.10, modules are grouped in collections
        1. References
            1. [builtin](https://docs.ansible.com/ansible/latest/collections/ansible/builtin/)
            2. [ansible examples](https://github.com/ansible/ansible-examples)
            3. [YAML](#ff-yaml) and [YAML Syntax](https://docs.ansible.com/ansible/latest/reference_appendices/YAMLSyntax.html#yaml-syntax)
        1. `ansible.builtin.command`: default module, do not support piping and redirects, use `shell` module
        2. `ansible.builtin.shell`
        3. `ansible.builtin.file`
            1. `dest`
            2. `state`
                1. `absent`: delete files or directories recursively
                2. `directory`: similir to `mkdir -p`
            3. `mode=600`
            4. `owner`
            5. `group`
        ...
        11. `ansible.builtin.setup`: get system information
    ...
    7. Playbook: ordered lists of tasks, written in YAML, can be run repeatedly
        1. Keywords
            ...
            3. `remote_user`: user account for the SSH connection
            4. `become: yes`
            5. `become_method: [sudo, su]`
            6. `vars`
            7. `tasks`: the units of an action in ansible, can be executed once with an ad hoc command
                1. Keywords
                    1. `name:`
                    2. `become: yes`
                    3. `the_module_name`
