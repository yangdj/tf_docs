...
1. `id()`
2.  `sys.getsizeof(): size of objects in bytes`
3. `type()`
4. `str()`
5. `repr()`
6. `dir()`
...
13. `class.mro()`
14. `class.__bases__`
15. `os.fspath(filename)`
16. `os.path.normcase(filename)`
17. `os.path.abspath(__file__)`
18. `os.path.dirname()`
19. `os.path.basename(dirname)`
20. `os.path.splitext(filename)`
21. `os.path.join(dirname, filename)`
22. `os.getpid()`
23. `threading.RLock(); lock.acquire(); lock.release()`
24. `threading.get_ident()`
25. `threading.current_thread().name`
26. `sys.modules.get('multiprocessing')`
27. `multiprocessing.current_process().name`
28. `time.time()`
...
1. `sys.exc_info() => (etype, value, tb)`, `tb` now can be get from exception's `__traceback__` attribute since Python3
2. `traceback.extract_tb(tb)`: return a `FrameSummary` object, which have following attributes:
...
3. `traceback.print_tb(tb)`
4. `traceback.format_tb(tb)`: return with strings, not print the result
5. `traceback.print_exception(etype, value, tb)`
6. `traceback.format_exception(etype, value, tb)`
7. `traceback.print_exc() <=> traceback.print_exception(*sys.exc_info())`
8. `trackback.format_exc()`
9.  `traceback.extract_stack()`
10. `traceback.format_stack()`
