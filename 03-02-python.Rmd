...
1. builtins
    ...
    13. `zip(a, b)`
    14. `ord()`: return the unicode code print for a one-character string
    15. `chr()`: return a Unicode character from number, 0 <= i <= 0x10ffff
    16. `int(x, base=10)`: `base=0` means to interpret the base from the string as an integer literal
    17. `hex()`: convert a number to hexadecimal strings
    18. `oct()`: conver to octal strings
    19. `bin()`: convert to binary strings
    20. `issubclass()`
    21. `isinstance()`: `isinstance(True, int) => True`
    22. `hasattr()`
    23. `getattr()`
    24. `setattr()`
    25. `class.__mro__`
...
3. Extensions
    ...
    4.  `(?aiLmsux)`: set the `A, I, L, M, S, U, or X` flag for the RE, embed in the front of pattern
    ...
    9.  `(?<=...)`: positive backward assertion, match if preceded by ... (must be fixed length).
    10. `(?<!...)`: negative backward assertion, match if not preceded by ... (must be fixed length).
    11. `(?(id/name)yes|no)` match yes pattern if the group with id/name matched, or no pattern if the group not matched, no part can be omitted
...
6. `re.Match` Object
    ...
    7. `..regs()`: return a tuple of span of entire group and subgroup
    8. `.re`: return the pattern to match
    9. `.string`: return the string to be matched
7. Notes
