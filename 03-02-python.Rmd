...
2. Concepts
    ...
    3. Inventory: a list of managed nodes, an inventory file is also called a hostfile
        1. Settings
            ...
            3. `-i path`: specify inventory path in commands, can use multiple times
        2. INI format
            1. hosts
                1. Types
                ...
                2. Host variables
                    ...
                    2. settings
                        ...
                        4. `ansible_connection=[local, ssh]`
            2. groups
                1. Naming
    ...
    7. Playbook: ordered lists of tasks, written in YAML, can be run repeatedly
3. Commands
    1. `ansible`
        1. Paramters
            1. `--version`
            2. pattern
                1. `all[*], ungroup`: for all hosts or just hosts not in group children
                2. `host1` - one host
                3. `host1:host[host1,host2]`: multiple hosts
                4. `webserver`: one group
                5. `websever:dbserver`: multiple group
                6. `webserver:!dbserver`: hosts in webserver, excluding in dbserver
                7. `webserver:&dbserver`: hosts in webserver and dbserver
            3. `-i | --inventory host-file`: can use multiple times
            4. `--limit[-l] pattern`: limits selected hosts to an additional pattern
            5. `--list-hosts`: outputs matching hosts, do not execute anything
            6. `-- playbook-dir basedir`
            7. `-m | --module-name moudule_name`: default command module
            8. `-a | --args "a1=v1 a2=v2"`: module arguments
            9. `-e | --extra-vars extra_vars`: key=value format, if filename prepend with @
            10. `--forks[-f] forks`: number of parallel process to use, default 5
