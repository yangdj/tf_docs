...
1. builtins
    ...
    12. `enumerate(a, start)`: `start` default 0, can be any integer
    13. `zip(a, b)`
    14. `int(x, base=10)`: `base=0` means to interpret the base from the string as an integer literal
    15. `ord()`: return the unicode code print for a one-character string
    16. `hex()`: convert a number to hexadecimal strings
    17. `oct()`: conver to octal strings
    18. `bin()`: convert to binary strings
    19. `issubclass()`
    20. `isinstance()`: `isinstance(True, int) => True`
    21. `hasattr()`
    22. `getattr()`
    23. `setattr()`
    24. `class.__mro__`
...
4.  sys
    ...
    7.  `sys.getsizeof(): size of objects in bytes`
    8.  `sys.modules.get('multiprocessing')`
    9.  `sys.exc_info() => (etype, value, tb)`: `tb` now can be get from exception's `__traceback__` attribute since Python3
5. traceback
...
6.  os
    1.  `os.fspath(filename)`
    2.  `os.path.normcase(filename)`
    3.  `os.path.abspath(__file__)`
    4.  `os.path.dirname()`
    5.  `os.path.basename(dirname)`
    6.  `os.path.splitext(filename)`
...
4. Operators
   ...
   The followings can be applied to all numeric types
...
6. Looping Statement
    1.  while: condition-controlled loop
        ...
        while condition:
            ...
            statement_n
        ```
    2.  for: collection-controlled loop
...
8. Functions
    ...
    A function ia a structuring element in programming languages to group a bunch of statements, so they can be utilized more than once.
    ...
    2.  Argument: a parameter passed to when call the function
    ...
    4.  `__doc__`: the first statement in the body of a function is usually a string statement called a Docstring
9.  Modules: Every file with file extension `.py` and proper python code can be seen or is a module.
    ...
    4.  `__name__`: a module can be run as a script as if it had been imported, with exception that its `__name__` set to `__main__`. So it's possible to program different behaviour into a module for two cases according to `__name__` value:
...
10. Packages
    ...
    A package is basically a directory with python files and a file with the name `__init__.py`, which can be empty or contains valid code. It structures module namespace using **dotted modules names**. `A.B` stands for a submodule named `B` in a package named `A`.

    An `__all__` list in the `__init__.py` can list module and packages names to be imported when import a package using `*` in the form `from package import *`.
...
13. Generator
    ...
    A generator is a function returning a generator object which is also an iterator (not vice versa) producing values lazily. The advantage of generators consist in automatically creating methods `__iter__()` and `__next__()`.
...
14. lambda
    ...
    The general syntax of a lambda function is : `lambda argument_list: expression`
...
Different data types can have different properties. These properties can be:

1. Collection
   
    Data contains several elements.
...
2. Sequential
   ...
   Elements are ordered in a defined sequence and they can be accessed via indices. The sequence implements `getitem()` and `len()` method. Python starts from 0.
   ...
   Functions:
    ...
    1. `+`: concatenate two sequence
    2. `*`: repeat sequence n times

3. Mutable
   
   whether the elements can be modified.

: (\#tab:py-data-types) Python Data Type Properties

| type      | collection | Seq | Mapping | mutable |
| --------- | ---------- | --- | ------- | ------- |
| bool      | no         | no  | no      | no      |
| int       | no         | no  | no      | no      |
| float     | no         | no  | no      | no      |
...
| str       | yes        | yes | no      | no      |
| bytes     | yes        | yes | no      | no      |
| bytearray | yes        | yes | no      | yes     |
| list      | yes        | yes | no      | yes     |
| tuple     | yes        | yes | no      | no      |
| dict      | yes        | no  | yes     | yes     |
| set       | yes        | no  | no      | yes     |
| frozenset | yes        | no  | no      | no      |
...
* bool: subclass of `int`
    ...
    2. `False`, following values will be considered as `False` value
        ...
        8. `{}`
        9. `None`
* int: unlimited size in Python3, no `long` type as in Python2
...
* str
    ...
    4.  Operations
        ...
        12. `.zfill(width)`: pad a numeric string with zeros on the left, the string is never truncated
        13. `.format(*args, **kwargs)`

            the string template contains one or more format codes (fields to be replaced) embedded in constant text. They are surrounded by `{}`. if a brace character has to be printed, it has to be escaped by doubling it, `{{`, `}}`.

            The format codes has structure `{index:[flags][options][width][.precision]type}`
...
* tuple: like list, but immutable and faster
    1. Initialization
        1. `s = ()`: empty tuple
        2. `s = (1, "hello", False)`
* dict:
    ...
    4. Methods
        ...
        2. `.values()`
        3. `.items()`
        4. `list(.keys()), list(.values()), list(.itmes())`: turn the result to list
...
2. Metacharacters
    1. `^`: mark the beginning of string, when in `re.MULTILINE` mode, match immediately after each newline
    2. `$`: mark the end of string, when in `re.MULTILINE` mode, match immediately before each newline
    3. `.`: match any character excpet newline, when in `re.S` mode, include the newline
    4. `[]`: match any character within it
    ...
    5. `*`: repeat 0 to any times
    6. `+`: repeat 1 to any times
    7. `?`: repeat 0 or 1 times
    8. `{m, n},, {n}, {n, }`: repeat between m and n times, n times, or no less than n times 
    9.  `(), (?P<hours>)`: numbered and named subgroup, `\1, \2, ..., (?P=hours)` back reference the subgroup, in `re.sub()`, use `\1, \g<1>, \g<hours>`
    10. `|`: alternation, match any part of it, `abc|ABC` matches `abc` or `ABC`
    11. `\\`: match a literal backslash `\`
3. Predefined Character Classes
    1. `\d`: `[0-9]`, match any decimal digit
    2. `\D`: `[^0-9]`, match complement of `\d`
    3. `\s`: `[ \t\n\r\f\v]`, match any whitespace character
    4. `\S`: `[^ \t\n\r\f\v]`, match complement of `\s`
    5. `\w`: `[a-zA-Z0-9_]`, match any alphanumeric character; with LOCALE, it will match the set `[a-zA-Z0-9_]` plus characters defined as letters for the current locale.
    6. `\W`: match complement of `\w`.
    7. `\b`: match the empty string, but only at the start or end of a word.
    8. `\B`: match the empty string, but not at the start or end of a word
4. Functions
    1. `re.compile(pattern, flags)`: for use the same regexp more than once
    ...
    3. `re.search()`: return Match object or None, return the first found substring
    4. `re.fullmatch()`: return Match object or None, match pattern to the entire string
    5. `re.findall()`: return a list of all non-overlapping matches in the string
    6. `re.finditer()`: return an iterator Match object
    7. `re.split()`: return a list of splited substrings
    8. `re.sub()`: return the replacement string
    9. `re.subn()`: return `(new_string, number)`
    10. `re.purge()`: clear the regular expression caches
5. `re.Match`
    ...
    6. `..regs()`: return a tuple of span of entire group and subgroup
    7. `.re`: return the pattern to match
    8. `.string`: return the string to be matched
