...
1. Environment Management
    ...
    2. conda
        1. version info: `conda --version`
        2. update: `conda update conda`
        3. list all environments: `conda info --envs`
        4. create virtual environment: `conda create --name vtl-name python=3.5`
        5. activate virtual environment: `conda activate vtl-name`
        6. quit virtual environment: `conda deactivate`
        7. remove virtual environment: `conda remove --name vtl-name --all`
        8. add new channel when `packages not available`: `conda config --append channels conda-forge`
        9. **Use Anoconda Prompt to execute when faild in cmd environment**
...
4. Syntax
    ...
    5.  Modules: Every file with file extension `.py` and proper python code can be seen or is a module.
    ...
    8. Iterable & Iterator
        ...
        while True:
            try:
                ...
                break
        ```
    9. Generator: A generator is a function returning a generator object which is also an iterator (not vice versa) producing values lazily. The advantage of generators consist in automatically creating methods `__iter__()` and `__next__()`. There are two types of generators:
    ...
    10. lambda: `lambda argument_list: expression`
    11. Decorators
    ...
    1. basic
        ...
        5. str
            ...
            3.  Escaping: use backslash `\`
                ...
                7. `\f`: FF, formfeed
                8.  `\v`: VT, Vertical Tab
                9.  `\r`: CR, Carrige Return
                10. `\ooo`: octal value `ooo`
                11. `\xhh`: hex value `hh`
                12. `\uxxxx`: 16-bit hex value `xxxx` (Unicode only)
                13. `\Uxxxxxxxx`: 32-bit hex value `xxxxxxxx` (Unicode only)
                14. `\N{name}`: named `name` in Unicode database (Unicode only)
            4.  Operations
                1. `.upper()`
                2. `.lower()`
                3. `.replace()`
                ...
                15. `.format(*args, **kwargs)`
                    ...
                    2. flags:
                        ...
                        2. `+`: a sign `+` or `-` will preceded the conversion, overrides a space flag
                        3. other padding character, should specify options
                    3. options:
                    ...
                    6. type:
                        ...
                        2. `c`: single character, accepts integer or single character string
                        3. `r`: string from `repr()`
        ...
        6. bytes
            5. immutable
            6.  a sequence of small integers. the elements are in the range 0 to 255
            7.  printed as a sequence of ASCII characters
            8. not possible to mix text and data in Python3, otherwise raise TypeError
            9.  `"hello".encode("UTF-8"); b"hello"`
            10. `b"hello".decode("UTF-8")`
        7. bytearray
            1. sequential
            2. mutable
        8. list:
